...
var source = {
    ...
    templates: {
        ...
        views: [`${paths.markup}**/*.*`, `${paths.scripts}**/*.jade`, `!${paths.markup}index.*`]
    },
    styles: {
        app: [paths.styles + '*.*'],
        themes: [paths.styles + 'themes/*', `${paths.scripts}**/*.less`],
        watch: [paths.styles + '**/*', `${paths.scripts}**/*.less`, '!' + paths.styles + 'themes/*']
    }
};
...
var injectOptions = {
    ...
    transform: function(filepath) {
        return `script(src=\'${filepath.substr(filepath.indexOf('app'))}?v=${uuidv()}\')`;
    }
};
...
gulp.task('vendor:base', getTask('vendor.base', {
    config: config,
    build: build,
    vendor: vendor,
...
gulp.task('vendor:app', getTask('vendor.app', {
    config: config,
    build: build,
    vendor: vendor,
    cssnanoOpts: cssnanoOpts,
...
// APP RTL
gulp.task('styles:app:rtl', getTask('styles.app.rtl', {
    config: config,
    source: source,
    build: build,
    cssnanoOpts: cssnanoOpts
...
gulp.task('templates:views', function() {
    ...
    if (config.useCache) {
        ...
        return gulp.src(source.templates.views)
            .pipe($.if(!config.isProduction, $.changed(build.templates.views, {
                extension: '.html'
            })))
            .pipe($.pug())
            .on('error', handleError)
...
gulp.task('watch', function() {
    ...
    gulp.watch(paths.static, gulpsync.sync(['assets:static:app:clean', 'assets:static:app']));
});
